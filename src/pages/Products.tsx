...
export default function Products() {
  ...
  const processedProducts = useMemo(() => {
    ...
    // Сортировка: недавно измененные -> Алфавит -> Размер
  ...
  const handleSyncProducts = async () => {
    ...
    if (!tokenContent || !tokenMarketplace) {
      return alert('ОШИБКА: Убедитесь, что в .env добавлены токены CONTENT и MARKETPLACE!');
    }
    ...
    try {
      ...
      const itemGroupMap = new Map<string, FbsStockItem>();
      ...
      await db.transaction('rw', db.fbsWarehouses, db.fbsStocks, db.fbsStatusHistory, async () => {
        ...
        finalStocksData.forEach(item => {
          ...
          if (!prev || prev.status !== currentStatus) newHistoryItems.push({ id: item.id, status: currentStatus, since: now });
        });
    ...
    <PageLayout>
      ...
      <Toolbar>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleSyncProducts} disabled={isLoading}>
              ...
              <div className="space-y-3">
                ...
                {modalPrices.map((p, index) => (
                  <div key={index} className="flex items-end gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200 group transition-colors hover:border-blue-300">
                    ...
                    <button onClick={() => handleRemovePricePeriod(index)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors mb-0.5" title="Удалить">
                      <Trash2 size={18} />
                    </button>
                  </div>
                ))}

                <button onClick={handleAddPricePeriod} className="flex items-center justify-center gap-2 w-full py-2.5 mt-2 border border-dashed border-gray-300 rounded-xl text-[13px] font-bold text-gray-500 hover:text-blue-600 hover:border-blue-300 hover:bg-blue-50 transition-all cursor-pointer">
